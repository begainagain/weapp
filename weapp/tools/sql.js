...
let selectRecords_test = function ( values ){
  let _sql = "select * FROM group_record WHERE group_id like ? and ((content_type='test' and dates=?) or content_type='normal') ORDER BY content_time DESC"
  // let data = await query(_sql)
  return query(_sql,values)
}
...
let selectRecords_history = function ( value ){
  let _sql = "select content FROM group_record WHERE user_id=? ORDER BY content_time DESC LIMIT 5"
  // let data = await query(_sql)
  return query(_sql,value)
}

let selectRecords_two = function ( values ){
  let _sql = "select content FROM group_record WHERE user_id=? and dates=? ORDER BY content_time DESC LIMIT 10"
  // let data = await query(_sql)
  return query(_sql,values)
}
...
let insertGroup_record = function( values ) {
  let _sql = 'insert into group_record(user_id,user_name,group_id,content,content_type,content_time,dates) VALUES(?,?,?,?,?,?,?)'
  return query(_sql, values)
}
